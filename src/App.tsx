...
function App () {
  ...
  const formatValue = (value: string) => {
    if (stringFormat === stringFormatEnum.Upercase) return value.toLocaleUpperCase()
    if (stringFormat === stringFormatEnum.Lowercase) return value.toLocaleLowerCase()
    if (stringFormat === stringFormatEnum.StartCase) {
      const array = value.toLocaleLowerCase().split(' ').map((word) => {
  ...
  const classNameCaseButton = 'border border-blue-200 active:bg-blue-400 py-1 px-6 rounded hover:bg-blue-200 hover:text-gray-800'
  const classNameUpercaseButton = classNameCaseButton + (stringFormatEnum.Upercase === stringFormat ? ' bg-blue-200 text-gray-800' : '')
  const classNameLowercaseButton = classNameCaseButton + (stringFormatEnum.Lowercase === stringFormat ? ' bg-blue-200 text-gray-800' : '')
  const classNameStartcaseButton = classNameCaseButton + (stringFormatEnum.StartCase === stringFormat ? ' bg-blue-200 text-gray-800' : '')
    ...
    <>
      <header>
        ...
        {avaliableLangs.map((lng: string) => (
          ...
          className={`border border-blue-200 active:bg-blue-400 py-1 px-4 rounded hover:bg-blue-200 hover:text-gray-800 justify-start text-xs ${i18n.resolvedLanguage === lng ? 'bg-blue-200 text-gray-800' : ''}`}
          type="submit" onClick={async () => await i18n.changeLanguage(lng)}>
              {lngs[lng].nativeName}
            </button>
        ))}
        </div>
      ...
      <main className='mt-8 border border-blue-200 rounded p-10 h-screen'>
        <section className='flex flex-row justify-around w-full'>
          <div className='flex flex-row flex-grow justify-start'>
            <form onSubmit={handleSubmit} className='flex flex-row justify-start gap-5'>
              <button className='border border-blue-200 active:bg-blue-400 py-1 px-6 rounded hover:bg-blue-200 hover:text-gray-800 justify-start' type='submit'>Xerar</button>
            <div className='flex flex-row flex-grow justify-start gap-1'>
              <input className='bg-blue-200 text-gray-800 text-center rounded w-12' type='text' min='1' max='10' value={profileNum} disabled/>
              <button onClick={handleClickMinus} className='border border-blue-200 active:bg-blue-400 py-1 px-4 rounded hover:bg-blue-200 hover:text-gray-800 justify-start' type='button'>-</button>
              <button onClick={handleClickPlus} className='border border-blue-200 active:bg-blue-400 py-1 px-4 rounded hover:bg-blue-200 hover:text-gray-800 justify-start' type='button'>+</button>
            </div>
            </form>
          </div>
          <div className='flex flex-row flex-grow justify-end gap-5'>
            <button onClick={handleClickUpercase} className={classNameUpercaseButton} type='button'>MAIÚSCULAS</button>
            <button onClick={handleClickLowercase} className={classNameLowercaseButton} type='button'>minúsculas</button>
            <button onClick={handleClickStartCase} className={classNameStartcaseButton} type='button'>Start Case</button>
          </div>
        </section>
        <section className='flex flex-col mt-12 gap-5'>
          ...
          <div className='flex flex-row gap-10 flex-wrap'>
            ...
            <Field width='w-72' labelName='Concello' value={formatValue('Santiago de Compostela')}/>
            <Field width='w-72' labelName='Localidade' value={formatValue('Santiago de compostela')}/>
          </div>
          <div className='flex flex-row gap-10 flex-wrap'>
            ...
            <Field width='w-96' labelName='Correo electrónico' value={'manuel.martinez@galicia.org'}/>
          </div>
